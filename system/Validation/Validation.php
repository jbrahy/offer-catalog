...
class Validation implements ValidationInterface {
	...
	public function withRequest(RequestInterface $request): ValidationInterface
	{
		...
		if (in_array($request->getMethod(), ['put', 'patch', 'delete'], TRUE) && strpos($request->getHeaderLine('Content-Type'), 'multipart/form-data') === FALSE)
		{
	...
	protected function processRules(string $field, string $label = NULL, $value, $rules = NULL, array $data = NULL): bool
	{
		...
		foreach ($rules as $rule)
		{
			...
			if ($passed === FALSE)
			{
				...
				$this->errors[$field] = is_null($error) ? $this->getErrorMessage($rule, $field, $label, $param, $value) : $error; // @phpstan-ignore-line
	...
	{
		$nonEscapeBracket = '((?<!\\\\)(?:\\\\\\\\)*[\[\]])';
		$pipeNotInBracket = sprintf('/\|(?=(?:[^\[\]]*%s[^\[\]]*%s)*(?![^\[\]]*%s))/', $nonEscapeBracket, $nonEscapeBracket, $nonEscapeBracket);
