...
class Pager implements PagerInterface {
	...
	protected function ensureGroup(string $group, int $perPage = NULL)
	{
		...
		$this->groups[$group] = [
			'uri' => clone current_url(TRUE), 'hasMore' => FALSE, 'total' => NULL, 'perPage' => $perPage ?? $this->config->perPage, 'pageCount' => 1, 'pageSelector' => $group === 'default' ? 'page' : 'page_' . $group,
		];
	...
	protected function calculateCurrentPage(string $group)
	{
		if (array_key_exists($group, $this->segment))
		{
			try
			{
				$this->groups[$group]['currentPage'] = (int)$this->groups[$group]['uri']->setSilent(FALSE)
