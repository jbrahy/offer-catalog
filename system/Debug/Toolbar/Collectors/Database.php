...
class Database extends BaseCollector {
	...
	public function display(): array
	{
		$data['queries'] = array_map(function (Query $query) {
			return [
				'duration' => ((float)$query->getDuration(5) * 1000) . ' ms', 'sql' => $query->debugToolbarDisplay(),
			];
		}, static::$queries);
	...
	protected function formatTimelineData(): array
	{
		...
		foreach ($this->connections as $alias => $connection)
		{
			// Connection Time
			$data[] = [
				'name' => 'Connecting to Database: "' . $alias . '"', 'component' => 'Database', 'start' => $connection->getConnectStart(), 'duration' => $connection->getConnectDuration(),
			];
		}
		...
		{
			$data[] = [
				'name' => 'Query', 'component' => 'Database', 'start' => $query->getStartTime(TRUE), 'duration' => $query->getDuration(),
			];
		}
