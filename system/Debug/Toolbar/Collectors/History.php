	...
	public function setFiles(int $current, int $limit = 20)
	{
		...
		foreach (array_reverse($filenames) as $filename)
		{
			...
			if (json_last_error() === JSON_ERROR_NONE)
			{
				...
				$files[] = [
					'time' => $time, 'datetime' => date('Y-m-d H:i:s', $time), 'active' => $time === $current, 'status' => $contents->vars->response->statusCode, 'method' => $contents->method, 'url' => $contents->offer_url, 'isAJAX' => $contents->isAJAX ? 'Yes' : 'No', 'contentType' => $contents->vars->response->contentType,
				];
			}
		}
