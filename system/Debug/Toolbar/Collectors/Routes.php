...
class Routes extends BaseCollector {
	...
	public function display(): array
	{
		...
		{
			$params[] = [
				'name' => $param->getName(), 'value' => $router->params()[$key] ?? '&lt;empty&gt;&nbsp| default: ' . var_export($param->isDefaultValueAvailable() ? $param->getDefaultValue() : NULL, TRUE),
			];
		}

		$matchedRoute = [
			[
				'directory' => $router->directory(), 'controller' => $router->controllerName(), 'method' => $router->methodName(), 'paramCount' => count($router->params()), 'truePCount' => count($params), 'params' => $params,
			],
		];
		...
		*/
		$routes = [];
		$methods = [
			'get', 'head', 'post', 'patch', 'put', 'delete', 'options', 'trace', 'connect', 'cli',
		];
		...
		foreach ($methods as $method)
		{
			...
			foreach ($raw as $route => $handler)
			{
				...
				{
					$routes[] = [
						'method' => strtoupper($method), 'route' => $route, 'handler' => $handler,
					];
				}
			}
		}
		...
		return [
			'matchedRoute' => $matchedRoute, 'routes' => $routes,
		];
	}
