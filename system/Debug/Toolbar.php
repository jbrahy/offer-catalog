...
class Toolbar {
	...
	public function prepare(RequestInterface $request = NULL, ResponseInterface $response = NULL)
	{
		...
		if (CI_DEBUG && ! is_cli())
		{
			...
			$script = PHP_EOL . '<script type="text/javascript" {csp-script-nonce} id="debugbar_loader" ' . 'data-time="' . $time . '" ' . 'src="' . site_url() . '?debugbar"></script>' . '<script type="text/javascript" {csp-script-nonce} id="debugbar_dynamic_script"></script>' . '<style type="text/css" {csp-style-nonce} id="debugbar_dynamic_style"></style>' . $kintScript . PHP_EOL;
	...
	public function run(float $startTime, float $totalTime, RequestInterface $request, ResponseInterface $response): string
	{
		...
		$data['vars']['response'] = [
			'statusCode' => $response->getStatusCode(), 'reason' => esc($response->getReason()), 'contentType' => esc($response->getHeaderLine('content-type')),
		];
	...
	protected function format(string $data, string $format = 'html'): string
	{
		...
		{
			$history = new History();
			$history->setFiles((int)Services::request()->getGet('debugbar_time'), $this->config->maxHistory);
