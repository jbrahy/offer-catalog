...
class Forge {
	...
	public function addForeignKey(string $fieldName = '', string $tableName = '', string $tableField = '', string $onUpdate = '', string $onDelete = '')
	{
		...
		$this->foreignKeys[$fieldName] = [
			'table' => $tableName, 'field' => $tableField, 'onDelete' => strtoupper($onDelete), 'onUpdate' => strtoupper($onUpdate),
		];
	...
	public function addField($field)
	{
		if (is_string($field))
		{
			if ($field === 'id')
			{
				$this->addField([
					'id' => [
						'type' => 'INT', 'constraint' => 9, 'auto_increment' => TRUE,
					],
				]);
				$this->addKey('id', TRUE);
			} else
	...
	protected function _createTable(string $table, bool $ifNotExists, array $attributes)
	{
		...
		$sql = ($ifNotExists) ? sprintf($this->createTableIfStr, $this->db->escapeIdentifiers($table)) : 'CREATE TABLE';
	...
	protected function _processFields(bool $createTable = FALSE): array
	{
		...
		foreach ($this->fields as $key => $attributes)
		{
			...
			$field = [
				'name' => $key, 'new_name' => $attributes['NAME'] ?? NULL, 'type' => $attributes['TYPE'] ?? NULL, 'length' => '', 'unsigned' => '', 'null' => '', 'unique' => '', 'default' => '', 'auto_increment' => '', '_literal' => FALSE,
			];
	...
	protected function _processForeignKeys(string $table): string
	{
		...
		if ($this->foreignKeys !== [])
		{
			foreach ($this->foreignKeys as $field => $fkey)
			{
				...
				$sql .= ",\n\tCONSTRAINT " . $this->db->escapeIdentifiers($nameIndex) . ' FOREIGN KEY(' . $this->db->escapeIdentifiers($field) . ') REFERENCES ' . $this->db->escapeIdentifiers($this->db->DBPrefix . $fkey['table']) . ' (' . $this->db->escapeIdentifiers($fkey['field']) . ')';
