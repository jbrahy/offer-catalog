...
class Connection extends BaseConnection {
	...
	public function connect(bool $persistent = FALSE)
	{
		...
		if (is_array($this->encrypt))
		{
			...
			if ( ! empty($ssl))
			{
				if (isset($this->encrypt['ssl_verify']))
				{
					...
					{
						defined('MYSQLI_OPT_SSL_VERIFY_SERVER_CERT') && $this->mysqli->options(MYSQLI_OPT_SSL_VERIFY_SERVER_CERT, 1);
					}
		...
		try
		{
			if ($this->mysqli->real_connect($hostname, $this->username, $this->password, $this->database, $port, $socket, $clientFlags))
			{
				...
				if (($clientFlags & MYSQLI_CLIENT_SSL) && version_compare($this->mysqli->client_info, 'mysqlnd 5.7.3', '<=') && empty($this->mysqli->query("SHOW STATUS LIKE 'ssl_cipher'")
						->fetch_object()->Value))
				{
					$this->mysqli->close();
	...
	public function error(): array
	{
		...
		{
			return [
				'code' => $this->mysqli->connect_errno, 'message' => $this->mysqli->connect_error,
			];
		}

		return [
			'code' => $this->connID->errno, 'message' => $this->connID->error,
		];
	}
