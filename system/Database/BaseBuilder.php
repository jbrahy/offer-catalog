...
class BaseBuilder {
	...
	public function orderBy(string $orderBy, string $direction = '', bool $escape = NULL)
	{
		...
		{
			$qbOrderBy[] = [
				'field' => $orderBy, 'direction' => $direction, 'escape' => FALSE,
			];
		} else
		{
			$qbOrderBy = [];
	...
	protected function whereHaving(string $qbKey, $key, $value = NULL, string $type = 'AND ', bool $escape = NULL)
	{
		...
		foreach ($key as $k => $v)
		{
			...
			$this->{$qbKey}[] = [
				'condition' => $prefix . $k . $v, 'escape' => $escape,
			];
		}
	...
	{
		$this->resetRun([
			'QBSelect' => [], 'QBJoin' => [], 'QBWhere' => [], 'QBGroupBy' => [], 'QBHaving' => [], 'QBOrderBy' => [], 'QBNoEscape' => [], 'QBDistinct' => FALSE, 'QBLimit' => FALSE, 'QBOffset' => FALSE,
		]);
	...
	{
		$this->resetRun([
			'QBSet' => [], 'QBJoin' => [], 'QBWhere' => [], 'QBOrderBy' => [], 'QBKeys' => [], 'QBLimit' => FALSE, 'QBIgnore' => FALSE,
		]);
	}
	...
	protected function _whereIn(string $key = NULL, $values = NULL, bool $not = FALSE, string $type = 'AND ', bool $escape = NULL, string $clause = 'QBWhere')
	{
		...
		$whereIn = [
			'condition' => $prefix . $key . $not . ($values instanceof Closure ? " IN ($ok)" : " IN :{$ok}:"), 'escape' => FALSE,
		];
	...
	protected function _like($field, string $match = '', string $type = 'AND ', string $side = 'both', string $not = '', bool $escape = NULL, bool $insensitiveSearch = FALSE, string $clause = 'QBWhere')
	{
		...
		foreach ($field as $k => $v)
		{
			...
			$this->{$clause}[] = [
				'condition' => $likeStatement, 'escape' => $escape,
			];
		}
	...
	protected function groupStartPrepare(string $not = '', string $type = 'AND ', string $clause = 'QBWhere')
	{
		...
		$where = [
			'condition' => $prefix . $not . str_repeat(' ', ++$this->QBWhereGroupCount) . ' (', 'escape' => FALSE,
		];
	...
	protected function groupEndPrepare(string $clause = 'QBWhere')
	{
		$this->QBWhereGroupStarted = FALSE;
		$where = [
			'condition' => str_repeat(' ', $this->QBWhereGroupCount--) . ')', 'escape' => FALSE,
		];
