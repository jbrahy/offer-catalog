...
class FileHandler extends BaseHandler {
	...
	public function __construct(Cache $config)
	{
		...
		{
			$config->file = [
				'storePath' => $config->storePath ?? WRITEPATH . 'cache', 'mode' => 0640,
			];
		}
	...
	public function getMetaData(string $key)
	{
		...
		return [
			'expire' => $data['time'] + $data['ttl'], 'mtime' => filemtime($this->path . $key), 'data' => $data['data'],
		];
	}
