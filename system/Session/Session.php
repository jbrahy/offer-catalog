...
class Session implements SessionInterface {
	...
	public function __construct(SessionHandlerInterface $driver, App $config)
	{
		...
		$this->cookie = new Cookie($this->sessionCookieName, '', [
			'expires' => $this->sessionExpiration === 0 ? 0 : time() + $this->sessionExpiration, 'path' => $cookie->path ?? $config->cookiePath, 'domain' => $cookie->domain ?? $config->cookieDomain, 'secure' => $cookie->secure ?? $config->cookieSecure, 'httponly' => TRUE, // for security
			'samesite' => $cookie->samesite ?? $config->cookieSameSite ?? Cookie::SAMESITE_LAX, 'raw' => $cookie->raw ?? FALSE,
		]);
	...
	protected function configure()
	{
		...
		$params = [
			'lifetime' => $this->sessionExpiration, 'path' => $this->cookie->getPath(), 'domain' => $this->cookie->getDomain(), 'secure' => $this->cookie->isSecure(), 'httponly' => TRUE, // HTTP only; Yes, this is intentional and not configurable for security reasons.
			'samesite' => $sameSite,
		];
