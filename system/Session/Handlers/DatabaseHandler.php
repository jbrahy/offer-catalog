...
class DatabaseHandler extends BaseHandler {
	...
	public function write($sessionID, $sessionData): bool
	{
		...
		{
			$insertData = [
				'id' => $sessionID, 'ip_address' => $this->ipAddress, 'timestamp' => 'now()', 'data' => $this->platform === 'postgre' ? '\x' . bin2hex($sessionData) : $sessionData,
			];
	...
	public function gc($maxlifetime): bool
	{
		...
		return ($this->db->table($this->table)
			->delete("timestamp < now() - INTERVAL {$interval}")) ? TRUE : $this->fail();
	}
